...
class Retriever:
    def __init__(self, doc_sources=[], embeddingModel="llama3") -> None:

        self.doc_sources = doc_sources
        self.embedding_model = embeddingModel

        self.docs = self.__load()
        self.chunks = self.__splits_chunks()
        self.embeddings = self.__embeddings()
        self.vector_store = self.__vector_store_load()
    ...
    def __load(self):
        ...
        loader = WebBaseLoader(
            web_paths=self.doc_sources,
            bs_kwargs={"parse_only": bs4_strainer},
        )
        docs = loader.load()
        return docs

    def __splits_chunks(self):
        """Indexing: split """
    ...
    def __vector_store_load(self):
        print('__vector_store_load')
        return Chroma.from_documents(documents=self.chunks, embedding=self.embeddings)
